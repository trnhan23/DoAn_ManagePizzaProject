...
public class MainQuanLyGUI extends JFrame {
    ...
    private void setFullScreen() {
        this.setExtendedState(JFrame.MAXIMIZED_BOTH); // Đặt cửa sổ ứng dụng vào chế độ toàn màn hình
        this.setUndecorated(true); // Ẩn các thành phần trang trí của cửa sổ (ví dụ: title bar, các nút điều khiển)

    }

    private void setSizeRelativeToScreen(double ratio) {
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        Insets screenInsets = Toolkit.getDefaultToolkit().getScreenInsets(this.getGraphicsConfiguration());
        int taskBarHeight = screenInsets.bottom; // Lấy chiều cao của thanh taskbar
        int width = (int) (screenSize.getWidth() * ratio);
        int height = (int) ((screenSize.getHeight() - taskBarHeight) * ratio); // Trừ đi chiều cao của thanh taskbar
    ...
    final Color clLeftItem = new Color(135, 206, 235);
    final Color clLeftItemHover = new Color(72, 88, 107);
    final Color clLeftItemSelected = new Color(205, 92, 92);
    ...
    private void addControls() {
        ...
        pnTitle.setBackground(new Color(0, 206, 209));

        btnDoiMatKhau = new JLabel("Chào "+DangNhapBUS.taiKhoanLogin.getTenDangNhap());
        btnDoiMatKhau.setFont(new Font("Tahoma", Font.PLAIN, 20));
        btnDoiMatKhau.setForeground(Color.WHITE);
        FontMetrics metrics = btnDoiMatKhau.getFontMetrics(btnDoiMatKhau.getFont());
        int textWidth = metrics.stringWidth(btnDoiMatKhau.getText());
        int textHeight = metrics.getHeight();
        
        btnDoiMatKhau.setToolTipText("Đổi mật khẩu");
        btnDoiMatKhau.setBounds(0, 0, textWidth>46?textWidth:46, 46);
...
//        JLabel lblAvatar = new JLabel(new ImageIcon("image/ManagerUI/pizza.png"), JLabel.CENTER);
//        lblAvatar.setPreferredSize(new Dimension(230, 210));
        ImageIcon imageIcon = new ImageIcon("image/ManagerUI/pizza-avata.png");

        // Lấy kích thước của JLabel
        Dimension labelSize = new Dimension(230, 210); // Kích thước của JLabel
        ...
        double scaleX = (double) labelSize.width / imgWidth;
        double scaleY = (double) labelSize.height / imgHeight;
        double scale = Math.min(scaleX, scaleY); // Tìm tỷ lệ giảm nhỏ nhất

        // Thay đổi kích thước của hình ảnh
        Image scaledImage = imageIcon.getImage().getScaledInstance((int) (imgWidth * scale), (int) (imgHeight * scale), Image.SCALE_SMOOTH);
        ...
        // Đặt kích thước của JLabel
        lblAvatar.setPreferredSize(labelSize);
    ...
    private void addEvents() {
        ...
        btnDoiMatKhau.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                DlgDoiMatKhau settingFrm = new DlgDoiMatKhau(); // Truyền vào form cha và true để hiển thị dialog modal
                settingFrm.setVisible(true);

                // Kiểm tra kết quả của dialog con
                if (settingFrm.logout == true) {
                    // Đóng form cha
                    dispose();
                    changLNF("Nimbus");
