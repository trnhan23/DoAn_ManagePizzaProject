...
public class PhanQuyenDAO {
    ...
    public ArrayList<PhanQuyen> getListQuyen() {
        try {
            ...
            while (rs.next()) {
                ...
                phanQuyen.setQlGiamGia(rs.getInt(7));
                dspq.add(phanQuyen);
            }
            return dspq;
        } catch (Exception e) {
    ...
    public PhanQuyen getQuyen(String quyen) {
        try {
            String sql = "SELECT * FROM PhanQuyen WHERE quyen= N'" + quyen + "'";
            Connection conn = new MyConnect().getConnection();
            Statement st = conn.createStatement();
            ResultSet rs = st.executeQuery(sql);
            if (rs.next()) {
    ...
    public boolean suaQuyen(PhanQuyen phanQuyen) {
        try {
            String sql = "UPDATE phanquyen SET NhapHang=?,QLSanPham=?,QLNhanVien=?,QLKhachHang=?,ThongKe=?, QLGiamGia=? WHERE Quyen=?";
            Connection conn = new MyConnect().getConnection();
            ...
            pre.setInt(6, phanQuyen.getQlGiamGia());
    ...
    public boolean themQuyen(PhanQuyen phanQuyen) {
        try {
            ...
            pre.setInt(7, phanQuyen.getQlGiamGia());
            return pre.executeUpdate() > 0;
        } catch (Exception e) {
        }
        return false;
